...
package com.media2359.acpdemo.tracking;
...
import com.media2359.acpdemo.Classifier.Recognition;
import com.media2359.acpdemo.env.BorderedText;
import com.media2359.acpdemo.env.ImageUtils;
import com.media2359.acpdemo.env.Logger;
...
public class MultiBoxTracker {
  ...
  public MultiBoxTracker(final Context context) {
    ...
    distanceTextPaint.setStyle(Style.FILL_AND_STROKE);
    distanceTextPaint.setTextSize(textSizePx);
  }
  ...
  public synchronized void draw(final Canvas canvas) {
    ...
    for (TrackedRecognition recognition : trackedObjects) {
      ...
      if (recognition.title.equals("elephant")) {
        RectF clonePos = new RectF(trackedPos);
        getFrameToCanvasMatrix().mapRect(clonePos);
        origin = new PointF(clonePos.centerX(), clonePos.centerY());
      }
    }
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      if (origin != null && !recognition.title.equals("elephant")) {
        canvas.drawLine(origin.x, origin.y, trackedPos.centerX(), trackedPos.centerY(), boxPaint);

        Path distanceTextPath = new Path();
        distanceTextPath.moveTo(origin.x, origin.y);
        distanceTextPath.lineTo(trackedPos.centerX(), trackedPos.centerY());

        double distance = Math.sqrt((origin.x - trackedPos.centerX()) * (origin.x - trackedPos.centerX()) + (origin.y - trackedPos.centerY()) * (origin.y - trackedPos.centerY()));
        canvas.drawTextOnPath(String.format("%.2f", distance), distanceTextPath, 150, -10, distanceTextPaint);
      }
